...
public:
    ...
    // 1 for static pixels that may supervise RGB/depth, 0 for dynamic pixels.
    torch::Tensor original_mask_;
    // Optional HxW integer class target (stored as float on CPU) and weight.
    // Class 0 is unknown/ignore; supervised classes are 1..C-1.
    torch::Tensor original_semantic_;
    torch::Tensor semantic_weight_;
    // Optional offline two-pass PCA/OpenCLIP supervision. Region id 0 is
    // unsupervised; positive ids index rows after subtracting one.
    torch::Tensor language_region_ids_;
    torch::Tensor language_basis_dot_;
    torch::Tensor language_mean_dot_;
    torch::Tensor language_confidence_;
